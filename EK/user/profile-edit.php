<?php
include '../db_conn.php';
session_start();

$user_id = $_GET['update'];
$username = $_SESSION["username"];       
$query = "SELECT * FROM `user_db` WHERE username='$username'";
$result = mysqli_query($conn, $query);
$row = mysqli_fetch_assoc($result);



// Check if form is submitted
if (isset($_POST['submit'])) {
    // Get user data from form
    $user_firstname = mysqli_real_escape_string($conn, $_POST['user_firstname']);
    $user_lastname = mysqli_real_escape_string($conn, $_POST['user_lastname']);
    $user_name = mysqli_real_escape_string($conn, $_POST['username']);
    $user_email = mysqli_real_escape_string($conn, $_POST['user_email']);
    $user_password = mysqli_real_escape_string($conn, $_POST['user_password']);


    // Check if user uploaded a new avatar
    if (!empty($_FILES['user_avatar']['name'])) {
        $avatar_name = $_FILES['user_avatar']['name'];
        $avatar_tmp_name = $_FILES['user_avatar']['tmp_name'];
        $avatar_size = $_FILES['user_avatar']['size'];
        $avatar_type = $_FILES['user_avatar']['type'];
        $avatar_ext = strtolower(pathinfo($avatar_name, PATHINFO_EXTENSION));

        // Allow certain file formats
        $allowed_ext = array("jpg", "jpeg", "png", "gif");

        // Check if file format is allowed
        if (in_array($avatar_ext, $allowed_ext)) {
            // Check if file size is less than 5MB
            if ($avatar_size < 5000000) {
                // Generate a unique filename for the avatar
                $avatar_new_name = uniqid('', true) . '.' . $avatar_ext;
                $avatar_upload_path = '../admin/uploadsUserAvatar/' . $avatar_new_name;

                // Move uploaded file to designated folder
                move_uploaded_file($avatar_tmp_name, $avatar_upload_path);

                // Delete previous avatar if it exists
                if (!empty($user_avatar)) {
                    unlink('../admin/uploadsUserAvatar/' . $user_avatar);
                }

                // Update user avatar in database
                $sql_update_avatar = "UPDATE user_db SET user_avatar = '$avatar_new_name' WHERE user_id = '$user_id'";
                $result_update_avatar = mysqli_query($conn, $sql_update_avatar);
            } else {
                // File size is too large
                echo "<p>File size too large. Maximum file size allowed is 5MB.</p>";
            }
        } else {
            // Invalid file format
            echo "<p>Invalid file format. Only JPG, JPEG, PNG and GIF files are allowed.</p>";
        }
    }

    // Update other user details in database
    $sql_update_user = "UPDATE user_db SET user_firstname = '$user_firstname', user_lastname = '$user_lastname', username = '$user_name', user_email = '$user_email', user_password = '$user_password' WHERE user_id = '$user_id'";
    $result_update_user = mysqli_query($conn, $sql_update_user);
    if ($result_update_user) {
        // Update success
        $message = 'User details updated successfully. Please login again!';
        $redirect_url = "profile.php?user_id=$user_id";
        echo "<script>alert('$message'); window.location.replace('index.php');</script>";
        exit;
    }else {
        echo "<p>Error updating user details. Please try again.</p>";
    }
}
?>


<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="./css/style.css">
    <link rel="stylesheet" href="https://unicons.iconscout.com/release/v4.0.8/css/line.css">
    <link rel="icon" href="data:image/jpeg;base64,">
    <!--Gg font-->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@300;400;500;600;700;800;900&display=swap" rel="stylesheet">
    <?php
        if(isset($_SESSION["username"])){
            echo "<title>Welcome, {$_SESSION['username']}</title>";        
        }
    ?>
    <style>
        
        #imagePreview {
            width: 200px;
            height: 200px;
            border: 1px solid #ccc;
            margin-bottom: 20px;
            background-size: cover;
            background-position: center;
        }
    </style>
</head>
<body>
    <nav>
        <div class="container nav__container">
            <a href="index.php"><h4>EK</h4></a>
            <ul class="nav__menu">
                <li><a href="course.php">Home</a></li>
                <li><a href="course.php">Topics</a></li>
                <li><a href="profile.php">Profile</a></li>
                <li><a href="signin.php">Log Out</a></li>

            </ul>
            <button id="open-menu-btn"><i class="uil uil-bars"></i></button>
            <button id="close-menu-btn"><i class="uil uil-multiply"></i></button>
        </div>
    </nav>


    <!--End nav-->
    <section class="form__section">
        <div class="container form__section-container">
            <h2>Edit User</h2>
            
            <form action="" method="POST" enctype="multipart/form-data">
                <h4>First name : &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<input type="text" placeholder="First Name"name="user_firstname" value="<?php echo $row['user_firstname']; ?>"></h4>
                <h4>Last name : &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<input type="text" placeholder="Last Name" name="user_lastname" value="<?php echo $row['user_lastname']; ?>"></h4>
				<h4>Username : &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<input type="text" placeholder="Username" name = "username" value="<?php echo $row['username']; ?>"></h4>
                <h4>Email : &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<input type="email" placeholder="Email" name="user_email" value="<?php echo $row['user_email']; ?>"></h4>
				<h4>Change password : <input type="password" placeholder="Change password"name="user_password" value="<?php echo $row['user_password']; ?>"></h4>
                

                <div class="form__control">
                    <label for="avatar"><h4>User Avatar :</h4></label>
                    <img id="imagePreview" src="../admin/uploadsUserAvatar/<?php echo $row['user_avatar']; ?>" >
                    <input type="file" id="avatar" name="user_avatar">
                </div>
                
                <button type="submit" class="btn btn-primary" name="submit">Save changes</button>
            </form>
        </div>
    </section>

	<footer>
        <div class="container footer__container">
            <div class="footer__1">
                <a href="index.php" class="footer_logo"><h4>EK</h4></a>
                <p>
                    Learning English more convinient, more excited and more happiest.
                </p>
            </div>
            <div class="footer__2">
                <h4>Permalinks</h4>
                <ul class="permalinks">
                    <li><a href="index.php">Home</a></li>
                    <li><a href="course.php">Topics</a></li>
                    <li><a href="contact.php">Contact</a></li>
                </ul>
            </div>

            <div class="footer__3">
                <h4>Privacy</h4>
                <ul class="privacy">
                    <li><a href="#">Privacy Policy</a></li>
                    <li><a href="#">Terms and conditions</a></li>
                    <li><a href="#">Refund Policy</a></li>
                    
                </ul>
            </div>

            <div class="footer__4">
                <h4>Contact Us</h4>
                <div>
                    <p>+84 369 247 326</p>
                    <p>phamhuythiep14@gmail.com</p>
                </div>
            

                <ul class="footer_socials">
                    <li>
                        <a href="https://www.facebook.com/profile.php?id=100008769240847"><i class="uil uil-facebook-f"></i></a>
                    </li>
                    <li>
                        <a href="https://www.instagram.com/phamhuythiep/"><i class="uil uil-instagram-alt"></i></a>
                    </li>
                    <li>
                        <a href="https://www.linkedin.com/in/ph%E1%BA%A1m-huy-thi%E1%BB%87p-undefined-949004270/"><i class="uil uil-linkedin-alt"></i></a>
                    </li>
                    <li>
                        <a href="https://github.com/phamhuythiep12"><i class="uil uil-github-alt"></i></a>
                    </li>
                </ul>
            </div> 
        </div>
    </footer>
    <script src="main.js"></script>
</body>
</html>