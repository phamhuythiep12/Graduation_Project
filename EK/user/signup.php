...
if (isset($_POST['submit'])) {
    ...
    if($firstname === "" && $lastname === "" && $username === "" && $user_email === "" && $user_password === ""){
        echo "<script>alert('Please full fill this form')</script>";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $query = "SELECT * FROM `user_db` WHERE user_email='$user_email'";
        $result = mysqli_query($conn, $query);
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if ($user_password != $confirm_password) {
                ...
                $sql = "INSERT INTO user_db (username, user_password, user_email, user_avatar, user_firstname, user_lastname)
                        VALUES ('$username', '$user_password', '$user_email', '$avatar_new_name', '$firstname', '$lastname')";
